...
class FileLogger extends \Nette\Object implements ILogger
{
	 ...
	 * @return void
	 */
	public function __construct(Conta $filenameMask = NULL, $minimumLogLevel = NULL, $defaultLogLevel = NULL, $logDir = NULL, $granularity = 0)
	{
		if ($filenameMask !== NULL)
			$this->filenameMask = $filenameMask;

		if ($minimumLogLevel === NULL)
//			$minimumLogLevel = \Nette\Config\Configurator::isProduction() ? ILogger::INFO : ILogger::DEBUG;
			$minimumLogLevel = ILogger::INFO;
	...
	private function validateLogLevel($level) {
		...
		if ($level !== FALSE && ($level > ILogger::LOGIN || $level < ILogger::EMERGENCY)) {
